...
export default function ToolList() {
  ...
  const liClass =
    "mx-2 grid h-10 w-10 place-items-center rounded-lg hover:bg-[#d1d1d6] dark:hover:bg-[#373738]";
  return (
    <div className="bg-[#dcdce1] dark:bg-[#2d2d2e]">
      <ul className="mt-4 flex flex-col gap-2">
        <Link to="/chat">
          <li className={liClass} onClick={() => setActiveItem("chat")}>
            <MessageCircle className={itemClass("chat")} />
          </li>
        </Link>
        <li className={liClass} onClick={() => setActiveItem("conv")}>
          <NotebookTabs className={itemClass("conv")} />
        </li>
        <li className={liClass} onClick={() => setActiveItem("login")}>
          <LogIn className={itemClass("login")} />
        </li>
        <Link to="/settings">
          <li className={liClass} onClick={() => setActiveItem("setting")}>
